...
autocmd BufNewFile *.sh,*.py,*.sv,*.svh,*.c exec ":call SetTitle()"
autocmd BufWrite *.sh,*.py,*.sv,*.svh,*.c exec ":call SetLastModifiedTime()"
func! SetTitle()
  let ex_name  = expand("%:e")
  let src_name = expand("%:t")
  let filename = toupper(substitute(src_name, '\.', '__', 'g'))
  let filetype = ""
  if ex_name == 'sh'
    let filetype = "bash"
  elseif ex_name == 'py'
  ...
  if (ex_name == 'sh') || (ex_name == 'py')
    call setline(line("$")  , "#!/usr/bin/env ".filetype)
    call setline(line("$")+1, "############################################################")
    call setline(line("$")+1, "# Filename      : ".src_name)
    call setline(line("$")+1, "# Author        : kiki")
    call setline(line("$")+1, "# Create        : ".strftime("%F %T"))
    call setline(line("$")+1, "# Last Modified : ".strftime("%F %T"))
    call setline(line("$")+1, "# Description   : This is a ".filetype." script.")
    call setline(line("$")+1, "############################################################")
    call setline(line("$")+1, "")
  elseif (ex_name == 'sv') || (ex_name == 'svh') || (ex_name == 'c')
    call setline(line("$")  , "/************************************************************")
    call setline(line("$")+1, "* Filename      : ".expand("%:t"))
    call setline(line("$")+1, "* Author        : kiki")
    call setline(line("$")+1, "* Create        : ".strftime("%F %T"))
    call setline(line("$")+1, "* Last Modified : ".strftime("%F %T"))
    call setline(line("$")+1, "* Description   : This is a production script.")
    call setline(line("$")+1, "************************************************************/")
    call setline(line("$")+1, "")
    call setline(line("$")+1, "`ifndef _".filename."_")
    call setline(line("$")+1, "`define _".filename."_")
    call setline(line("$")+1, "")
    call setline(line("$")+1, "")
    call setline(line("$")+1, "")
    call setline(line("$")+1, "`endif")
  endif
endfunc

func SetLastModifiedTime()
  let lineno = search('^\A Last Modified', '', line(10))
  let mod_time = strftime("%F %T")
  if lineno > 0
    let line = getline(lineno)
    let line = substitute(line, ':\s\+\d\{4}\(-\d\{2}\)\{2}\s\+\(\d\{2}:\)\{2}\d\{2}', ": ".mod_time, "g")
    call setline(lineno, line)
  else
    echoe "No line containing modified time in header comment."
  endif
endfunc
